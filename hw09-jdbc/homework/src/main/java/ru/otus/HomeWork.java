...
// Сделайте тоже самое с классом Manager (для него надо сделать свою таблицу)
        ...
        managerSecond.setLabel("managerSecondButChanged");
        var managerSecondChanged =  dbServiceManager.saveManager(managerSecond);
        var managerSecondChangedSelect = dbServiceManager.getManager(managerSecondChanged.getNo())
                .orElseThrow(() -> new RuntimeException("Manager not found, id:" + managerSecondChanged.getNo()));
        log.info("managerSecondChangedSelect:{}", managerSecondChangedSelect);

        // ------get list All Manager---------
        var managerSelectedAll = dbServiceManager.findAll();
        managerSelectedAll.stream().forEach(manager ->
                log.info("List Manager:{}",manager)
        );
    }
